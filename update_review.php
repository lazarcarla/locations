...
if (isset($_POST[ 'submit' ])) {
    ...
    $update_review = $conn->prepare( "UPDATE reviews SET rating = ?, title = ?, description = ? WHERE id_review = ?" );
...
<body>
    ...
    <section class="account-form">
        ...
        <?php
        $select_review = $conn->prepare( "SELECT * FROM reviews WHERE id_review = ? LIMIT 1" );
        ...
        if ($select_review->rowCount() > 0) {
            while ( $fetch_review = $select_review->fetch( PDO::FETCH_ASSOC ) ) {
                ...
                <form action="" method="post">
                    ...
                    <a href="locationReview.php?get_id=<?= $fetch_review[ 'id_location' ]; ?>" class="option-btn">go back</a>
                </form>
                <?php
                }
            }
